...
struct SwapExactAmountOutTestCase {
    name: &'static str,
    pre_operations: Vec<OrderOperation>,
    token_out: Coin,
    token_in_denom: &'static str,
...
fn test_swap_exact_amount_out() {
    ...
    let base_denom = "base";
    let sender = Addr::unchecked("sender");
    let test_cases: Vec<SwapExactAmountOutTestCase> = vec![
        SwapExactAmountOutTestCase {
            name: "invalid in denom",
            pre_operations: vec![],
            token_out: coin(100u128, quote_denom),
            token_in_denom: "notadenom",
        ...
        SwapExactAmountOutTestCase {
            name: "invalid out denom",
            pre_operations: vec![],
            token_out: coin(100u128, "notadenom"),
        ...
        SwapExactAmountOutTestCase {
            name: "invalid swap fee",
            pre_operations: vec![],
            token_out: coin(100u128, base_denom),
            token_in_denom: "notadenom",
            token_in_max_amount: Uint128::from(100u128),
            swap_fee: Decimal::one(),
            expected_output: coin(100u128, quote_denom),
    ...
    for test in test_cases {
        ...
        // -- System under test --
        let response = dispatch_swap_exact_amount_out(
            deps.as_mut(),
            sender.to_string(),
            test.token_in_denom.to_string(),
            test.token_in_max_amount,
            test.token_out,
            test.swap_fee,
        );
